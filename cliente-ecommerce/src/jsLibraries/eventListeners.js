  ...
  logBoard.addEventListener('click', async (e) => {
    ...
    switch (e.target.value) {
      ...
      case 'loguin':
        ...
        const user = await formFetchtData({
          route: `/api/session/loguin/${updateData.email}/log/${updateData.password}`,
          method: 'GET',
        });
        if (user.status == 'succes') {
          // Si esta bien SETEAR EL PAYLOAD EN COOKIES SESSION
          // Redirección al home
          window.location.href = `/views/products`;
        } else {
      ...
      case 'github':
        ...
        const userload = await formFetchtData({
          route: `/api/session/github/${catchedData.email}/log/${catchedData.password}`,
          method: 'GET',
        });
...
onePageHeader.addEventListener('change', async (e) => {
  if (e.target.checked == true) {
    ...
    if (e.target.id == '.orderHeigh') {
      // Bloquea orderLow
      orderCheckbox = document.querySelector('#orderLow');
    } else if (e.target.id == 'orderLow') {
      // Bloquea orderHeigh
...
onePageMain.addEventListener('click', async (e) => {
  ...
  switch (e.target.value) {
    ...
    case 'cargar-producto':
      ...
      await upLoadData({
        apiRoute: `/api/products/mongo`,
        method: 'POST',
        formId: e.target.parentNode.id,
        updatableData: nodosId,
      });
    ...
    case 'update-producto':
      ...
      await upLoadData({
        apiRoute: `/api/products/mongo/${e.target.id}`,
        method: 'PUT',
        formId: e.target.parentNode.id,
    ...
    // CART Cases
    case 'clear-cart':
      let clearing = await await formFetchtData({
        route: `/api/carts/658388103a44d83d3749d1d6`,
        method: 'DELETE',
      });
    ...
    case 'substract-of-cart':
      ...
      await formFetchtData({
        route: `/api/carts/${cartId}/product/${e.target.parentNode.parentNode.id}`,
        info: information,
        method: 'PUT',
      });
      // socket = !undefined && socket.emit('update-product-db', 'change done');
    ...
    case 'add-to-cart':
      ...
      break;
    case 'delete-of-cart':
      let deleting = await await formFetchtData({
